            ...
            <tbody>
                @foreach ($students as $student)
                    <tr>
                        ...
                        <td>{{ $student->name }}</td>
                        <td>{{ $student->email }}</td>
                        <td>{{ $student->status == 0 ? 'Belum aktif' : 'Aktif' }}</td>
                        <td>
                            <div class="d-flex">
                                @if($student->status == 0)
                                <button type="button" class="btn badge text-bg-success text-decoration-none me-1" data-bs-toggle="modal" data-bs-target="#active-modal" onclick="bindingActive({{ $student->id }}, '{{ $student->name }}', '{{ $student->email }}')">Aktifkan</button>
                                @endif
                                <a href="{{ route('students.edit', $student->id) }}" class="btn badge text-bg-secondary text-decoration-none">Edit</a>
                                <button type="button" class="btn badge text-bg-danger text-decoration-none ms-1" data-bs-toggle="modal" data-bs-target="#delete-modal" onclick="bindingDelete('{{ $student->id }}')">Hapus</button>
                            </div>
                        </td>
                    </tr>
                @endforeach
...
@push('scripts')
    <script>
        ...
        function bindingDelete(id) {
            $('#delete-form').attr('action', `{{ url('students') }}/${id}`)
        }
